...
const amount = document.querySelector(".amount input");
 ...
 dropdowns.forEach((select)=> {  //to fill dropdown
...
button.addEventListener("click", async (evt)=>{         //to acess input and controlling the input
  evt.preventDefault();
...
msg.innerText=`${amtVal} ${fromCurr} = ${finalAmt} ${toCurr}`; //the updated message
  ...
  //to make the arrow icon work 
exBtn.addEventListener("click", ()=>{
  temp= fromCurr.value;
  fromCurr.value=toCurr.value; //swapping value
  toCurr.value=temp;

  updateFlag(fromCurr);  //swappes flags
  updateFlag(toCurr);
  button.click();

  button.click(); //triggers the button F
});
